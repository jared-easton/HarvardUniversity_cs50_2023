...
def buy():
    ...
    if request.method == "POST":
        ...
        flash(f"Secured new share(s) of stock!")
        return redirect("/")
    else:
...
def register():
    ...
    if request.method == "POST":
        ...
        if len(rows) != 0:
            return apology("Please Select Different Username.", 400)
...
def sell():
    ...
    if request.method == "POST":
        if not request.form.get("symbol") or not request.form.get("shares"):
            return apology("Enter a Ticker Symbol & Amount of Shares")
        elif not request.form.get("shares").isdigit():
        ...
        stock = lookup(symbol)
        if not stock:
            return apology("Wrong Ticker Symbol")
        ...
        for row in rows:
            if row["symbol"] == symbol:
                if shares > row["totalShares"]:
                    return apology("Select Fewer Shares")
        ...
        flash(f"You just sold share(s) of stock!")
        return redirect("/")
...
def account():
    ...
    if request.method == "POST":
        ...
        return render_template("resetpasswordsuccess.html", success=1)
